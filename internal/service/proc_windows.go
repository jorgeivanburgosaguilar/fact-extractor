...
// interrupt reaches our handler and the shutdown is ours to order — first the
// in-flight request fails, then Stop() takes the service down deliberately.
...
// runner holding ~4.7 GB of VRAM, which is the exact failure this tool
